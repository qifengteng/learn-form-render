...
export const createStore = (...args) => {
    ...
    const buildProps = (field, componentInstance) => {
        const originProps = field.componentProp || {};
        const props = {
            ...originProps,
            clearable: isDef(originProps.clearable)
                ? originProps.clearable
                : true,
    ...
    const buildEvents = (field, componentInstance) => {
        const originEvents = field.componentEvent || {};
        const events = {
            ...originEvents,
            input: (value) => {
        ...
        if (field.component === "el-input" && field.trimSpace !== false) {
            events["blur"] = () => {
                if (typeof componentInstance.modelValue === "string") {
                    componentInstance.modelValue =
                        componentInstance.modelValue.trim();
                }
            };
        }
        return events;
    ...
    const buildAttrs = (field) => {
        const defPlaceholder = defaultPlaceholder[field.component];
        return {
            placeholder: isDef(field.placeholder)
                ? field.placeholder
                : defPlaceholder,
        };
    };
    ...
    const buildChildren = (h, field, componentInstance) => {
        if (componentInstance.options && componentInstance.options.length) {
            return componentInstance.options.map((opt) =>
                buildOptionComponent(h, field, opt)
            );
        }
    };
